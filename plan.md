...
- Callback execution rechecks abort immediately after backend begin and before invoking user code. Durable Object SQLite performs the equivalent check after workerd admits the native storage transaction, so an operation aborted while waiting for acquisition or transaction serialization does not enter the callback.
- Singleton-pool connection cleanup is owned by pool closure, not transaction completion. Internally owned connections use their normal `close`; caller-supplied SQLite3 clients configure a no-op pool cleanup and remain usable after wrapper connection and pool closure. Bounded explicit transactions retain their separate acquired-resource release behavior.
...
- Ownership regression coverage proves singleton transaction completion does not invoke pool cleanup and singleton pool closure invokes its configured cleanup. Existing direct and generic SQLite3 integration tests prove a real supplied client remains usable after wrapper closure. The original PR #183 real SQLite3 rollback-failure test and the focused core rollback-precedence test both continue to protect preservation of the original callback error.
...
- The new core abort tests were first run against the previous behavior and failed because callback aborts committed and explicit post-begin aborts committed. The later ownership and abort-during-begin regression tests also failed against the previous behavior before implementation. After implementation, the focused core pool and transaction suites passed all 65 tests. `npm run fix`, `npm run lint`, and the unified `npm run build:ts` passed on 2026-09-09. Driver integration/runtime tests were intentionally not run by the implementation agent at the user's request; the targeted and aggregate integration/e2e gates above remain pending user results.
...
- An async transaction callback that never settles cannot be forcibly stopped by `AbortSignal`. Existing guards and connection closures retain resource ownership until the underlying callback settles, preventing unsafe connection reuse, but the database transaction or writer resource can remain occupied. Do not introduce a naive abort race that releases the resource while stale callback code can still resume.
