...
import { identityMapCacheProvider, pongoCacheWrapper } from './cacheWrapper';
...
describe('pongoCacheWrapper — key prefixing', () => {
  ...
  it('get prefixes key', () => {
    const provider = identityMapCacheProvider();
  ...
  it('getMany prefixes all keys', () => {
    ...
    const wrapper = pongoCacheWrapper({ provider });
    wrapper.getMany([`${dbName}:${collectionName}:a`, `${dbName}:${collectionName}:b`]);
  ...
  it('setMany prefixes all keys', () => {
    ...
    const wrapper = pongoCacheWrapper({ provider });
    wrapper.setMany([{ key: `${dbName}:${collectionName}:a`, value: { _id: 'a' } }]);
  ...
  it('deleteMany prefixes all keys', () => {
    ...
    const wrapper = pongoCacheWrapper({ provider });
    wrapper.deleteMany([`${dbName}:${collectionName}:a`, `${dbName}:${collectionName}:b`]);
    expect(spy).toHaveBeenCalledWith(['mydb:users:a', 'mydb:users:b']);
  });
});
...
describe('pongoCacheWrapper — error swallowing', () => {
  ...
  it('set silently succeeds when provider throws', async () => {
    ...
    const wrapper = pongoCacheWrapper({ provider });
    await expect(wrapper.set(`${dbName}:${collectionName}:x`, { _id: 'x' })).resolves.toBeUndefined();
  });
  ...
  it('delete silently succeeds when provider throws', async () => {
    ...
    const wrapper = pongoCacheWrapper({ provider });
    await expect(wrapper.delete(`${dbName}:${collectionName}:x`)).resolves.toBeUndefined();
  });
  ...
  it('getMany returns empty array when provider throws', async () => {
    ...
    const wrapper = pongoCacheWrapper({ provider });
    expect(await wrapper.getMany([`${dbName}:${collectionName}:a`, `${dbName}:${collectionName}:b`])).toEqual([undefined, undefined]);
  });
  ...
  it('setMany silently succeeds when provider throws', async () => {
    ...
    const wrapper = pongoCacheWrapper({ provider,  });
    await expect(wrapper.setMany([{ key: `${dbName}:${collectionName}:x`, value: { _id: 'x' } }])).resolves.toBeUndefined();
  });
  ...
  it('deleteMany silently succeeds when provider throws', async () => {
    ...
    const wrapper = pongoCacheWrapper({ provider,  });
    await expect(wrapper.deleteMany([`${dbName}:${collectionName}:x`])).resolves.toBeUndefined();
  });
});
