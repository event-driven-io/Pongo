...
describe('SQLite MongoDB Compatibility Tests', () => {
  let mf: Miniflare;
  ...
  describe('Delete Operations', () => {
    ...
    it('should not restore collection dropped in session when transaction is aborted', async () => {
      const docs = [
        { name: 'David', age: 40 },
        { name: 'Eve', age: 45 },
        { name: 'Frank', age: 50 },
      ...
      const tables = await pongoDb.sql.query<{ name: string }>(
        SQL`SELECT name FROM sqlite_master WHERE type = 'table' AND name = ${'dropInAbortedTransaction'}`,
      );
