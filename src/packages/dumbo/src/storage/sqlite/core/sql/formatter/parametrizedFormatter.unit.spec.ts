...
describe('SQLite Parametrized Formatter', () => {
  describe('format method', () => {
    ...
    it('handles identifiers by inlining them', () => {
    ...
    it('should mix identifiers and parameters correctly', () => {
    ...
    it('handles arrays by expanding to individual parameters', () => {
    ...
    it('handles special value types', () => {
    ...
    it('throws error for non-parametrized SQL', () => {
      assert.throws(() => {
  ...
  describe('formatRaw method', () => {
    it('should return inline formatted SQL string', () => {
      const sql = SQL`SELECT * FROM users WHERE id = ${123} AND name = ${'John'}`;
  ...
  describe('mapSQLValue method', () => {
    it('handles basic types', () => {
      assert.strictEqual(
        sqliteFormatter.valueMapper.mapValue(123, JSONSerializer),
        123,
      );
      assert.strictEqual(
        sqliteFormatter.valueMapper.mapValue('test', JSONSerializer),
        'test',
      );
      assert.strictEqual(
        sqliteFormatter.valueMapper.mapValue(null, JSONSerializer),
        null,
      );
      assert.strictEqual(
        sqliteFormatter.valueMapper.mapValue(undefined, JSONSerializer),
        null,
      );
    });

    it('handles SQLite-specific type conversions', () => {
      // Boolean conversion
      assert.strictEqual(
        sqliteFormatter.valueMapper.mapValue(true, JSONSerializer),
        true,
      );
      assert.strictEqual(
        sqliteFormatter.valueMapper.mapValue(false, JSONSerializer),
        false,
      );
      ...
      const date = new Date('2023-01-01T00:00:00.000Z');
      assert.strictEqual(
        sqliteFormatter.valueMapper.mapValue(date, JSONSerializer),
        '2023-01-01T00:00:00.000Z',
      );
      ...
      const bigint = BigInt(123456789012345);
      assert.strictEqual(
        sqliteFormatter.valueMapper.mapValue(bigint, JSONSerializer),
        '123456789012345',
      );
    });

    it('handles SQL identifier type', () => {
    ...
    it('handles nested SQL', () => {
      ...
      const result = sqliteFormatter.valueMapper.mapValue(
        nestedSql,
        JSONSerializer,
      );
      assert.strictEqual(typeof result, 'string');
    ...
    it('handles complex types', () => {
      const obj = { key: 'value' };
      const objResult = sqliteFormatter.valueMapper.mapValue(
        obj,
        JSONSerializer,
      );
